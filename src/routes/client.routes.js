        ...
        const history = client.screeningSessions.map((session) => ({
            ...
            anthropometry: session.anthropometryAssessment
                ? {
                    ...
                    waistCircumferenceCm: session.anthropometryAssessment.waistCircumferenceCm,
            ...
            glucose: session.biochemicalAssessment
                ? {
                    fastingGlucoseMgDl: session.biochemicalAssessment.fastingGlucoseMgDl,
                    postprandialGlucoseMgDl: session.biochemicalAssessment.postprandialGlucoseMgDl,
            ...
            clinicalSymptoms: session.clinicalAssessment
                ? {
                    ...
                    frequentUrinationNight: session.clinicalAssessment.frequentUrinationNight,
            ...
            screeningResult: session.screeningResult
                ? {
                    ...
                    finalScreeningCategory: session.screeningResult.finalScreeningCategory,
            ...
            medication: session.medicationAssessment
                ? {
                    usesHypertensionDrug: session.medicationAssessment.usesHypertensionDrug,
                    usesOralAntidiabetic: session.medicationAssessment.usesOralAntidiabetic,
                    usesInsulin: session.medicationAssessment.usesInsulin,
                    hypertensionDrugName: session.medicationAssessment.hypertensionDrugName,
                    antidiabeticDrugName: session.medicationAssessment.antidiabeticDrugName,
...
clientRoutes.post("/:clientId/screenings", async (req, res) => {
    try {
        ...
        const waistCircumferenceCm = body.anthropometry?.waistCircumferenceCm !== undefined &&
        ...
        const systolicBp = body.clinical?.systolicBp !== undefined &&
            body.clinical?.systolicBp !== null
            ? Number(body.clinical.systolicBp)
            : null;
        const diastolicBp = body.clinical?.diastolicBp !== undefined &&
            ...
            data: {
                ...
                biochemicalAssessment: {
                    create: {
                        ...
                        postprandialGlucoseMgDl: body.biochemical?.postprandialGlucoseMgDl ?? null,
                ...
                clinicalAssessment: {
                    create: {
                        systolicBp,
                        diastolicBp,
                        bloodPressureStatus: getBloodPressureStatus(systolicBp, diastolicBp),
                    },
                },
            },
            include: {
...
function getWaistStatus(gender, waistCm) {
    if (!waistCm)
        return null;
...
function getHba1cStatus(hba1cPercent) {
    if (hba1cPercent === null || hba1cPercent === undefined)
        return null;
    if (hba1cPercent >= 6.5)
        return "DIABETES_RISK";
    if (hba1cPercent >= 5.7)
...
function getBloodPressureStatus(systolicBp, diastolicBp) {
