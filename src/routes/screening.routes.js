...
screeningRoutes.post("/:clientId/anthropometry", async (req, res) => {
    try {
        ...
        if (waist !== null) {
            ...
            else if ((client.gender.toLowerCase() === "female" ||
...
screeningRoutes.get("/:screeningId/clinical-analysis", async (req, res) => {
    try {
        ...
        const hypertensionDiagnosis = screening.clinicalAssessment.bloodPressureStatus === "Normal" ||
...
screeningRoutes.post("/:screeningId/medication", async (req, res) => {
    try {
        const screeningId = Number(req.params.screeningId);
        const { hypertensionDrugName, antidiabeticDrugName, medicationNotes } = req.body;
        const screening = await prisma.screeningSession.findUnique({
            where: { screeningId },
        });
        if (!screening) {
        ...
        const hasHypertensionDrug = hypertensionDrugName &&
            hypertensionDrugName !== "Tidak menggunakan" &&
            hypertensionDrugName !== "";
        const hasAntidiabeticDrug = antidiabeticDrugName &&
            antidiabeticDrugName !== "Tidak menggunakan" &&
            antidiabeticDrugName !== "";
        const usesInsulin = antidiabeticDrugName &&
        ...
        if (usesInsulin) {
            await prisma.screeningResult.upsert({
                ...
                update: {
                    ...
                    referralReason: "Patient uses insulin. Further consultation with an internal medicine doctor and dietitian/nutritionist is recommended.",
                },
                create: {
                    screeningId,
                    referralRequired: true,
...
screeningRoutes.post("/:screeningId/physical-activity", async (req, res) => {
    try {
        ...
        if (!screening.anthropometryAssessment) {
            return res.status(400).json({
                message: "Anthropometry data is required before calculating energy needs",
            });
        }
...
screeningRoutes.post("/:screeningId/anthropometry-weekly", async (req, res) => {
    try {
        ...
        const weight = Number(weightKg);
        const height = Number(heightCm);
        const waist = waistCircumferenceCm !== undefined ? Number(waistCircumferenceCm) : null;
        const heightMeter = height / 100;
        const bmi = weight / (heightMeter * heightMeter);
        let bmiStatus = "Normal";
        if (bmi < 18.5) {
