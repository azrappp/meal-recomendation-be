...
const FASTAPI_BASE_URL = process.env.FASTAPI_BASE_URL || "https://menu-api-rust.vercel.app";
const WEEKLY_RULES = {
    LH: 3,
    LN: 4,
    S: 3,
...
function simplifyWeeklyMenu(savedWeeklyMenu) {
    return {
...
function groupItemsByMealSimple(items) {
    const mealOrder = [
        "breakfast",
        "morning_snack",
        "lunch",
...
function buildExcludedFoodNames(categoryUsedFoodCounts) {
    const excluded = [];
    for (const [categoryCode, foodCounts] of Object.entries(categoryUsedFoodCounts)) {
        const maxRepeat = WEEKLY_RULES[categoryCode] ?? 7;
        for (const [foodName, count] of Object.entries(foodCounts)) {
            const useHardExclusion = categoryCode === "LH" || categoryCode === "S" || categoryCode === "B";
...
function buildAllowedFoodNamesForDay(day) {
    const alwaysAllowed = [
        // MP
        "Nasi",
        "Nasi beras merah",
...
function updateWeeklyHistory(weeklyHistory, dailyMenu) {
    const foodsUsedToday = new Map();
        ...
        return res.status(200).json({
            ...
            data: {
                ...
                screeningResult: {
                    ...
                    hypertensionStatus: screening.screeningResult.hypertensionStatus,
                    obesityStatus: screening.screeningResult.obesityStatus,
                    finalScreeningCategory: screening.screeningResult.finalScreeningCategory,
                },
                energyRequirement: {
                    dailyEnergyKcal: screening.energyRequirement.dailyEnergyKcal,
                    carbohydrateGram: screening.energyRequirement.carbohydrateGram,
...
function buildDietType(status) {
    ...
    if (hasDM && hasHT && hasObesity)
        return "DM_HT_OBESITY";
    if (hasDM && hasHT)
        return "DM_HT";
    if (hasDM && hasObesity)
        return "DM_OBESITY";
...
function getSodiumLimitByDietType(dietType) {
    if (dietType.includes("HT")) {
        return 2000;
    }
    return 2300;
}
function getFiberMinimumByDietType(dietType) {
...
async function saveGeneratedMenu(params) {
    const { screeningId, dietType, fastApiPayload, generatedMenu, startDate } = params;
                ...
                create: [
                    {
                        ...
                        items: {
                            create: generatedMenu.meals.flatMap((meal) => meal.items.map((item) => ({
...
async function saveGeneratedWeeklyMenu(params) {
    const { screeningId, dietType, fastApiPayload, generatedMenus, startDate } = params;
        ...
        data: {
            ...
            days: {
                create: generatedMenus.map((menu, index) => ({
                    ...
                    items: {
                        create: menu.meals.flatMap((meal) => meal.items.map((item) => ({
                            mealTime: meal.meal_time,
                            foodName: item.food_name,
                            categoryCode: item.category_code,
                            portion: item.portion,
...
mealRecommendationRoutes.get("/menu/:menuRecommendationId", async (req, res) => {
    try {
        const menuRecommendationId = Number(req.params.menuRecommendationId);
        if (!Number.isInteger(menuRecommendationId) ||
...
mealRecommendationRoutes.delete("/menu/:menuRecommendationId", async (req, res) => {
    try {
        const menuRecommendationId = Number(req.params.menuRecommendationId);
        if (!Number.isInteger(menuRecommendationId) ||
            menuRecommendationId <= 0) {
...
async function recalculateMenuDaySummary(menuDayId) {
...
mealRecommendationRoutes.patch("/items/:menuItemId/eaten", async (req, res) => {
    try {
        const menuItemId = Number(req.params.menuItemId);
        const { isEaten, userNote } = (req.body ?? {});
        if (!Number.isInteger(menuItemId) || menuItemId <= 0) {
            return res.status(400).json({
                message: "Valid menuItemId is required",
            });
...
mealRecommendationRoutes.get("/clients/:clientId/menu-by-date", async (req, res) => {
    try {
        const clientId = Number(req.params.clientId);
        ...
        return res.status(200).json({
            ...
            data: {
                client: {
                    clientId: menuDay.menuRecommendation.screeningSession.client.clientId,
                    fullName: menuDay.menuRecommendation.screeningSession.client.fullName,
...
function groupItemsByMealForResponse(items) {
    const mealOrder = [
        "breakfast",
        "morning_snack",
        "lunch",
...
mealRecommendationRoutes.get("/clients/:clientId/menu-dates", async (req, res) => {
    try {
        const clientId = Number(req.params.clientId);
        if (!Number.isInteger(clientId) || clientId <= 0) {
            return res.status(400).json({
